 ...
 */
@Component
public class CheckOrderPayStatusTask {
	
    //记录日志
    private static Logger logger = LoggerFactory.getLogger(CheckOrderPayStatusTask.class);
	...
	public void checkOrderPayStatus() {
		logger.info("run start to CheckOrderPayStatus.");
		try {
			OrderPo op = new OrderPo();
			op.setStatus(ConstValue.ORDER_WAITING_FOR_PAYMENT);
			List<OrderPo> orderList = mainService.getOrderByAllParam(op);
			for (int i = 0 ; i < orderList.size() ; i ++ ) {
				OrderPo order = orderList.get(i);
				//订单支付状态查询
				List<OrderPayDO> payOrderList = mainService.queryPayOrderListByOrderId(order.getOrderId());
					...
					for(OrderPayDO orderPayDO : payOrderList) {
						...
						if(!"PAY_SUCCESS".equals(orderPayDO.getStatus())) {
							if("wxpay".equals(productCode)) {//检查微信支付接口
								payFlag = queryWeiXinOrderStatus(orderPayDO,order.getUid(),productCode);
						    } else if("alipay".equals(productCode)) {//检查支付宝支付接口
						    	payFlag = queryAliPayOrderStatus(order.getUid(),orderPayDO);
						    }
						} else {
							hasSuccessFlag = true;
						}
						...
						if(payFlag || hasSuccessFlag) {
							/** 修改房源订单状态 */
							updateOrderStatus(orderPayDO.getOrderId(),order.getUid());
						}
					}
				} 
				
			}
			logger.info("run end to CheckOrderPayStatus.");
		} catch (Exception e) {
			logger.error("CheckOrderPayStatus() 定时任务出错.",e);
		}
	}
