...
public class Mssql58Dialect extends Dialect {
	...
	public String getLimitString(String sql, int offset, int limit) {
		...
		pagingSelect.append("select * from (select row_number()over(order by tempcolumn)temprownumber,* ").append(" from (select top ").append(offset + limit - 1).append(" tempcolumn=0,* from ( ");
		// 暂不支持多排序结果合并查询
