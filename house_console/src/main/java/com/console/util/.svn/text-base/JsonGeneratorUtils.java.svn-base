...
public class JsonGeneratorUtils {
	...
	private static ObjectMapper objectMapper = null;   
	 public static    JsonGenerator createRetMaptJSONObject(ServletResponse response,boolean success,String message) throws IOException{  
		    response.setCharacterEncoding("utf-8");
			response.setContentType("application/json;charset=UTF-8");
		    objectMapper=new ObjectMapper();
		    jsonGenerator=objectMapper.getFactory().createGenerator(response.getWriter());	
			jsonGenerator.writeStartObject();
		    jsonGenerator.writeObjectField("success", success);
		    jsonGenerator.writeObjectField("message", message);
