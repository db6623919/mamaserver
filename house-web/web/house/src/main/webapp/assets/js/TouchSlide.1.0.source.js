...
var TouchSlide = function(a){
	...
	var opts = {
		...
		swipeScroll: a.swipeScroll || false, //每次切换结束时判断，是否因为内容太长而回到顶部
		resetWrapper: a.resetWrapper || null //每次切换结束时，是否要重置ul外层div的高度以防止过多的空白出现
	}
	...
	for ( var i =0; i<conBoxSize; i++ ){  
			...
			var cWidth=chi.clientWidth;
			var oWidth=chi.offsetWidth;
			if(cWidth>640)
			{
				cWidth=640;
			}
	...
	var doPlay=function(isTouch){
		...
		//切换时是否回到顶部
		var target = document.querySelector(opts.slideCell);
		if(opts.swipeScroll && document.body.scrollTop > target.offsetTop)
		{
			backToTab('body', target.offsetTop);
		}

		//切换时是否要重置ul外层div的高度
		if(opts.resetWrapper && loaded)
		{
			var current_ul = $(opts.resetWrapper).find('ul:eq('+index+')');
			var ul_height = current_ul.height();
			var set_height;
			console.log(ul_height)

			ul_height < $(window).height() ? set_height = $(window).height() : set_height = ul_height;
			$(opts.resetWrapper).css({'height': set_height});
		}
		loaded = true;
		doStartFun();
	...
	//详情页回到tab顶部
	var backToTab = function(wrapper, top)	
	{
		$(wrapper).animate({
			'scrollTop': top
		}, 200, 'linear');
	...
	$('#top').click(function(){
		backToTab('body', document.querySelector(opts.slideCell).offsetTop)
	})
