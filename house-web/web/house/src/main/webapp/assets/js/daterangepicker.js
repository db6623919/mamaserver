...
function($, moment) {
    ...
    var DateRangePicker = function(element, options, cb) {
        ...
        var DRPTemplate = '<div class="daterangepicker dropdown-menu" style="width:100%; top:0px;">' 
        	+ '<div style="position:absolute;z-index:-1;width:100%; height:20px; background:#000; top:0px; '+(options.noBanner?'display:none;':'')+'">' 
        	+ '<header>' 
        	+ '<nav>' 
        	+ '<div class="title list-top">'
/*        	+'<a name="date_start">选择日期</a>-'
        	+'<a name="date_end">选择日期</a>'
        	+'<a style="float:right;" onclick="reset()" id="dateClearUp" href="#"></a></div>' 
        	+ '<a class="back left" id="rollback" rel="nofollow" href="javascript:void(0);"></a>' */
   /*         +'<div class="title">选择入住日期</div>'
            +'<a class="left" rel="nofollow" href="order-index.html">取消</a>'
            +'<a class="right"  id="confirm-datehear"href="order-index1.html">确定</a>'*/
        	+ '</nav>' 
        	+ '</header>' 
        	+ '</div>' 
         ...
         '</div>' + '</div>' + '<div class="clear"></div>' + '<div class="calendar right"></div>' + '<div class="calendar left"></div>'+'<div class="calendar third"></div>' +'<div class="calendar fourth"></div>' + '</div>';
    ...
    DateRangePicker.prototype = {
        ...
        clickDate: function(e) {
            ...
            if (cal.hasClass('left')) {
                ...
                if (spDate < checkInTime || spDate > checkOutTime || spDate < currDate) {
                	return;
                }
                ...
                if (spDate < checkInTime || spDate > checkOutTime) {
                    return;
                }
                //update by tylerdd
            ...
            //update by 2015-10-13 tylerdd 第三次点击时重置已选日期
            ...
            //update by 2015-09-27 tylerdd
            if (cClose) {  
            	var bookNum=$DTU.getDateMargin(stringToDate(clickBegin),stringToDate(clickEnd));
        ...
        renderCalendar: function(calendar, selected, minDate, maxDate, month) {
            ...
            for (var row = 0; row < 6; row++) {
                ...
                for (var col = 0; col < 7; col++) {
                    ...
                    var checkInTime = $("#checkInTime").val();
                    var checkOutTime = $("#checkOutTime").val();
                    cname += (spDate >= checkInTime && spDate <= checkOutTime && spDate >= currDate) ? '': 'off';
                    var isoff = false;
                    if (!(spDate >= checkInTime && spDate <= checkOutTime && spDate >= currDate)) {
                    	isoff = true;
                    }
                    if ((minDate && calendar[row][col].isBefore(minDate)) || (maxDate && calendar[row][col].isAfter(maxDate))) {
                        cname = ' off disabled ';
                    } else if (calendar[row][col].format('YYYY-MM-DD') == selected.format('YYYY-MM-DD') && !isoff) {
                        // cname += ' active ';
                    ...
                    } else if (calendar[row][col] >= this.startDate && calendar[row][col] <= this.endDate && !isoff) {
                     				if (sDay!=''&&calendar[row][col].format('YYYY-MM-DD') ==  clickBegin) { 
	                            cname += ' start-date'; 
	                            sDay='入住';
	                            down=true;
                    ...
                    //update by tylerdd 2015-10-15
                    if(special&&special[spDate]&&sDay!='离开' && !isoff){
...
/*                    		if(sDay!=''&&spDate==currDate){
                    			sDay=sDay+'<b id="b-'+spDate+'" class="perice-off '+(down?"perice-down":"")+'"><i>'+special[spDate].price+'</i><input type="hidden" value="'+special[spDate].value+'"/></b>'
                    		}
                            else */
                            if(sDay!=''){
